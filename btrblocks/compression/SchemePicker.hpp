...
class CSchemePicker {
 public:
  ...
  static SchemeType& chooseScheme(StatsType& stats, u8 allowed_cascading_level, double* return_max_compression_ratio=nullptr) {
    ...
    if (MyTypeWrapper::getOverrideScheme() != autoScheme()) {
      ...
      if(return_max_compression_ratio != nullptr){
        *return_max_compression_ratio = max_compression_ratio;
      }
  ...
  static void compress(const Type* src,
                       ...
                       std::shared_ptr<StatsType> columnStats = nullptr) {
  
    Log::debug("Compressing with max level {}", allowed_cascading_level);
    ThreadCache::get().compression_level++;
    StatsType stats = columnStats == nullptr ? StatsType::generateStats(src, nullmap, tuple_count) : *columnStats;
    SchemeType* preferred_scheme = nullptr;
    ...
    if(force_scheme==autoScheme() && (allowed_cascading_level == 0 || tuple_count == 0)) {
      Log::debug(MyTypeWrapper::getTypeName() + ": UNCOMPRESSED");
      preferred_scheme = &MyTypeWrapper::getScheme(SchemeCodeType::UNCOMPRESSED);
      after_size = preferred_scheme->compress(src, nullmap, dest, stats, 0);
      scheme_code = CB(preferred_scheme->schemeType());
    }
    else if (force_scheme==autoScheme() && (ThreadCache::get().estimation_level == 0 && ThreadCache::get().compression_level == 1) &&
    ...
    else if (force_scheme==autoScheme() && MyTypeWrapper::shouldUseFOR(stats.min) && allowed_cascading_level > 1) {
