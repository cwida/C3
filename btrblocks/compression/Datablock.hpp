...
class Datablock : public RelationCompressor {
 public:
  ...
  static vector<u8> compress(const InputChunk& input_chunk, const u8& force_scheme = autoScheme());
  static u32 writeMetadata(const std::string& path,
                           std::vector<ColumnType> types,
                           vector<u32> part_counters,
                           u32 num_chunks);

  static SIZE compress(const InputChunk& input_chunk, u8* output_buffer, const u8& force_scheme = autoScheme(), int* used_scheme = nullptr, int* get_nullmap_size = nullptr, std::shared_ptr<c3::ColumnStats> columnStats = nullptr);
};
// -------------------------------------------------------------------------------------
}  // namespace btrblocks
// -------------------------------------------------------------------------------------
