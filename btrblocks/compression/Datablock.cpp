...
// -------------------------------------------------------------------------------------
std::vector<u8> Datablock::compress(const InputChunk& input_chunk, const u8& force_scheme) {
  ...
  auto total_size = compress(input_chunk, output.data(), force_scheme);
  // Resize the output vector to the actual used size
  output.resize(total_size);
  return output;
}
// -------------------------------------------------------------------------------------
// TODO: add force scheme parameter
SIZE Datablock::compress(const InputChunk& input_chunk, u8* output, const u8& force_scheme, int* used_scheme, int* get_nullmap_size, std::shared_ptr<c3::ColumnStats> columnStats) {
  ...
  switch (input_chunk.type) {
    case ColumnType::INTEGER: {
      std::shared_ptr<btrblocks::SInteger32Stats> IntStats = columnStats == nullptr ? nullptr : columnStats->intStats;
    ...
    case ColumnType::DOUBLE: {
      // -------------------------------------------------------------------------------------
      std::shared_ptr<btrblocks::DoubleStats> DoubleStats = columnStats == nullptr ? nullptr : columnStats->doubleStats;
    ...
    case ColumnType::STRING: {
      // -------------------------------------------------------------------------------------
      // Collect stats
      StringStats stats = columnStats == nullptr ? StringStats::generateStats(StringArrayViewer(input_chunk.data.get()),
                                                     input_chunk.nullmap.get(),
                                                     input_chunk.tuple_count, input_chunk.size) : *(columnStats->stringStats);
      // -------------------------------------------------------------------------------------
      // Make decisions
      StringScheme* preferred_scheme;
      if(force_scheme != 255){
        preferred_scheme = &TypeWrapper<StringScheme, StringSchemeType>::getScheme(force_scheme);
      }
      else{
        preferred_scheme = &StringSchemePicker::chooseScheme(stats, cfg.strings.max_cascade_depth);
      }

      auto foo = preferred_scheme->schemeType();
      // Update meta data
  ...
  if(used_scheme){*used_scheme = meta->compression_type;};
  ...
  if(get_nullmap_size != nullptr){
    *get_nullmap_size = nullmap_size;
  }
