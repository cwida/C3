...
struct NumberStats {
 public:
  ...
  const T* src;
  const BITMAP* bitmap;
  std::map<T, std::pair<u32,u32>> distinct_values; // pair: dict code, frequency
  T min;
  T max;
  NumberStats() = delete;
  // -------------------------------------------------------------------------------------
  ...
  static NumberStats generateStatsBasic(u32 tuple_count) {
    NumberStats stats(nullptr, nullptr, tuple_count);
    // -------------------------------------------------------------------------------------
  ...
  static NumberStats generateStats(const T* src, const BITMAP* nullmap, u32 tuple_count) {
    ...
    for (u64 row_i = 0; row_i < tuple_count; row_i++) {
      ...
      if (stats.distinct_values.find(current_value) == stats.distinct_values.end()) {
        stats.distinct_values.insert({current_value, {stats.unique_count, 1}});
        stats.unique_count++;
      } 
      else {
        stats.distinct_values[current_value].second++;
        // stats.max_distinct_value_occurence = std::max(stats.max_distinct_value_occurence, stats.distinct_values[current_value]);
      }
      ...
      // if((nullmap == nullptr || nullmap[row_i]) && stats.first_non_null_idx == -1){
