...
struct SchemeConfig {
  ...
  struct {
    ...
    IntegerSchemeType rle_force_values_scheme{IntegerSchemeType::BP};
    // in integer RLE, override the scheme used for run lengths with
    // this scheme instead of using the scheme picking algorithm
    IntegerSchemeType rle_force_counts_scheme{IntegerSchemeType::BP};
  } integers;
  // ------------------------------------------------------------------------------
  struct {
    // maximum percentage of unique elements in a block
  ...
  struct {
    ...
    uint8_t fsst_codes_max_cascade_depth{1};
    // in FSST, override the scheme used for the codes with
    // this scheme instead of using the scheme picking algorithm
    IntegerSchemeType fsst_force_codes_scheme{IntegerSchemeType::BP};
  } strings;
  // ------------------------------------------------------------------------------
